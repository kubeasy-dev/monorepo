...
2. **What challenge type fits?** `fix` (something is broken), `operate` (run an operational task), `improve` (harden a working setup), or `migrate` (transform an existing setup)
